// pages/Spam.tsx - Spam Emails Page
// Shows spam emails from spam_emails collection
// v1.0: Initial implementation based on Sent.tsx
...
const SpamPage = () => {
  ...
  // Fetch spam emails from Firestore
  const { emails, loading: emailsLoading, error: emailsError } = useSpamEmails(currentUser?.uid);
  ...
  const handleEmailClick = (email: Email) => {
    setSelectedEmail(email as unknown as SpamEmail);
  };
  ...
  const handleLongPress = useCallback((_email: Email) => {
    setIsSelectionMode(true);
  }, []);

  // Handle batch delete (placeholder - will implement later)
  const handleBatchDelete = useCallback(async () => {
    if (checkedEmails.size === 0) return;
    
    // TODO: Implement spam delete API
    console.log('Delete spam emails:', Array.from(checkedEmails));
          ...
          <nav className="flex-shrink-0 border-b border-zinc-700/50">
            ...
            <div className="flex lg:hidden items-center justify-between h-14 px-3">
              ...
              <div className="flex items-center">
                ...
                {/* Page Title */}
                <span className="text-white font-medium text-sm">Spam</span>
              </div>
            ...
            <div className="hidden lg:flex items-center justify-between px-6 pt-4">
              ...
              <div className="flex items-center gap-4 pb-4">
                ...
                <span className="text-[#8FA8A3] font-medium text-sm">Spam</span>
            ...
            >
              {selectedEmail && (
                <div className="p-6 pt-8">
                  <p className="text-zinc-500 text-xs uppercase tracking-wider mb-2">Category</p>
                  <p className="text-red-400 text-sm font-medium">SPAM</p>
                  <p className="text-zinc-500 text-xs mt-4 uppercase tracking-wider mb-2">From</p>
                  <p className="text-zinc-400 text-xs truncate">
                    {selectedEmail.senderEmail || selectedEmail.from}
                  </p>
                </div>
              )}
            </div>
