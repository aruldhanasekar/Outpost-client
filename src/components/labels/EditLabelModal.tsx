...
interface EditLabelModalProps {
  ...
  labelId: string;
  labelName: string;
  initialAutoLabel?: boolean;
  initialAutoLabelEmails?: string[];
}
...
export function EditLabelModal({ 
  ...
  onLabelUpdated,
  labelId,
  labelName: initialLabelName,
  initialAutoLabel = false,
  initialAutoLabelEmails = []
}: EditLabelModalProps) {
  ...
  const [autoLabel, setAutoLabel] = useState(initialAutoLabel);
  const [selectedEmails, setSelectedEmails] = useState<string[]>(initialAutoLabelEmails);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setAutoLabel(initialAutoLabel);
      setSelectedEmails(initialAutoLabelEmails);
      setLabelNameError(null);
    }
  }, [isOpen, initialLabelName, initialAutoLabel, initialAutoLabelEmails]);
  ...
  useEffect(() => {
    ...
    fetchLabels();
  }, [currentUser, isOpen, initialLabelName]);
  ...
  useEffect(() => {
    ...
    // No need to check if name hasn't changed
    if (labelName.trim().toLowerCase() === initialLabelName.toLowerCase()) {
      setLabelNameError(null);
      setIsCheckingName(false);
      return;
    }
  ...
  }, [labelName, existingLabels, initialLabelName]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      >
        ...
        <div className="p-6 pt-8">
          {/* Title */}
          <h2 className="text-lg font-semibold text-white mb-5">Edit Label</h2>
